...
#include "nav2_static_planner/planner_server.hpp"

using namespace std::chrono_literals;

namespace nav2_static_planner
{

PlannerServer::PlannerServer()
: nav2_util::LifecycleNode("nav2_static_planner", "", true),
  ...
  costmap_(nullptr)
{
  ...
  costmap_ros_ = std::make_shared<nav2_costmap_2d::Costmap2DROS>(
    "static_global_costmap", std::string{get_namespace()}, "static_global_costmap");
...
PlannerServer::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  current_pose_ = std::make_shared<geometry_msgs::msg::Point32>();
  // current_pose_->x = 1.5;
  // current_pose_->y = 9.5;
  // current_pose_->z = 0.0;
  auto callback = [this](const PolygonStamped::SharedPtr msg) -> void {
    footprint_to_pose(msg, current_pose_);
    RCLCPP_DEBUG(get_logger(), "footprint_to_pose: %f,%f",
      current_pose_->x, current_pose_->y);
  };

  footprint_sub_ = node->create_subscription<PolygonStamped>(
...
void
PlannerServer::footprint_to_pose(
  const PolygonStamped::SharedPtr footprint,
  geometry_msgs::msg::Point32::SharedPtr pose)
{
  std::vector<geometry_msgs::msg::Point32> points = footprint->polygon.points;
  double sum_x = 0.0;
  double sum_y = 0.0;
  ...
  pose->x = sum_x/points.size();
  pose->y = sum_y/points.size();
}
...
PlannerServer::computePlan()
{
  ...
  try {
    ...
    start.pose.position.x = current_pose_->x;
    start.pose.position.y = current_pose_->y;
    ...
    RCLCPP_INFO(
      get_logger(),
      "Found valid path of size %u from (%.2f, %.2f) to (%.2f, %.2f)",
